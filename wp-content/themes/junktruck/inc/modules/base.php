...
if ( ! class_exists( 'JunkTruck_Module_Base' ) ) {

	/**
	 * Define JunkTruck_Module_Base class
	 */
	abstract class JunkTruck_Module_Base {
		...
		public function load_child_modules( $childs = array() ) {
			...
			foreach ( $childs as $child => $childs_of_child ) {
				junktruck_theme()->load_module( $child, $childs_of_child );
			}
