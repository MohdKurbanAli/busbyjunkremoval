...
if ( ! class_exists( 'JunkTruck_Blog_Layouts_Module' ) ) {

	/**
	 * Define JunkTruck_Blog_Layouts_Module class
	 */
	class JunkTruck_Blog_Layouts_Module extends JunkTruck_Module_Base {
		/**
		 * properties.
		 */
		private $layout_type;
		...
		public function filters() {
			...
			add_filter( 'junktruck-theme/customizer/blog-sidebar-enabled', array( $this, 'customizer_blog_sidebar_enabled' ) );
			add_filter( 'junktruck-theme/customizer/blog-columns-enabled', array( $this, 'customizer_blog_columns_enabled' ) );
			add_filter( 'junktruck-theme/posts/template-part-slug', array( $this, 'apply_layout_template' ) );
			add_filter( 'junktruck-theme/posts/post-style', array( $this, 'apply_style_template' ) );
			add_filter( 'junktruck-theme/posts/list-class', array( $this, 'add_list_class' ) );
			add_filter( 'junktruck-theme/site-content/container-enabled', array( $this, 'disable_site_content_container' ) );
		...
		public function module_init_properties() {
			...
			$this->layout_type 		= junktruck_theme()->customizer->get_value( 'blog_layout_type' );
			$this->layout_columns 	= junktruck_theme()->customizer->get_value( 'blog_layout_columns' );
		...
		public function customizer_options( $options ) {
			...
			$new_options = array(
				'blog_layout_type' => array(
					...
					'field'    => 'select',
					'choices'  => array(
						'default'          => esc_html__( 'Listing', 'junktruck' ),
						'grid'             => esc_html__( 'Grid', 'junktruck' ),
						'vertical-justify' => esc_html__( 'Vertical Justify', 'junktruck' ),
					),
					'type' => 'control',
				)
			);
