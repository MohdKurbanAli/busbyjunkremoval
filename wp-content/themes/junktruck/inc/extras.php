 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'junktruck_set_post_meta_value' );

/**
 * Container type
 */
add_filter( 'theme_mod_container_type', 'junktruck_set_post_meta_value' );

/**
 * Header layout type
 */
add_filter( 'theme_mod_header_layout_type', 'junktruck_set_header_layout_value' );
 ...
 * @return string
 */
function junktruck_set_post_meta_value( $value ) {
	$queried_obj = junktruck_get_queried_obj();

	if ( ! $queried_obj ) {
		return $value;
	}

	$meta_key   = 'junktruck_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * @return string
 */
function junktruck_set_header_layout_value( $value ) {

	if ( wp_is_mobile() ) {
		return 'mobile';
	}

	return junktruck_set_post_meta_value( $value );
}
 ...
 */
function junktruck_get_queried_obj() {
	$queried_obj = apply_filters( 'junktruck-theme/posts/queried_object_id', false );

	if ( ! $queried_obj && ! junktruck_maybe_need_rewrite_mod() ) {
		return false;
	}
 ...
 * @return boolean
 */
function junktruck_maybe_need_rewrite_mod() {
 ...
 * @return string
 */
function junktruck_render_macros( $string ) {

	$macros = apply_filters( 'junktruck-theme/data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function junktruck_render_icons( $content ) {
	$icons     = junktruck_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'junktruck_render_icons_callback', $content );
}
 ...
 * @return string
 */
function junktruck_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function junktruck_get_render_icons_set() {
	return apply_filters( 'junktruck-theme/icons/icons-set', array(
 ...
 * @return string
 */
function junktruck_render_theme_url( $url ) {
 ...
 * @return string
 */
function junktruck_justify_thumbnail_size( $mask = 0, $thumbnail_size = 'post-thumbnail', $justify_size='junktruck-thumb-justify', $justify_size_1 = 'junktruck-thumb-justify', $justify_size_2 = 'junktruck-thumb-justify-2') {
	$mask_list = array(
 ...
 * @return string
 */
function junktruck_get_post_template_part_slug() {
	return apply_filters( 'junktruck-theme/posts/template-part-slug', 'template-parts/content' );
}
 ...
 * @return string
 */
function junktruck_get_post_style() {
	return apply_filters( 'junktruck-theme/posts/post-style', false );
}
 ...
 * @return array
 */
function junktruck_kses_post_allowed_html( $additional_allowed_html = array() ) {
 ...
 * @return bool
 */
function junktruck_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, junktruck_theme()->customizer->get_default( $meta ) );
