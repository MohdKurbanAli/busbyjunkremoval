 ...
 */
function junktruck_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->junktruck_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function junktruck_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->junktruck_comment_list_args['avatar_size'] ) ) {
		$size = $comment->junktruck_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'junktruck-theme/comments/author-avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function junktruck_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'junktruck_get_comment_author_link', sprintf( '<span class="fn h6-style">%1$s</span>', get_comment_author_link() ), $args );
}
 ...
 */
function junktruck_get_comment_date( $args = array() ) {
	$format = get_option( 'date_format' ) . ' ' . esc_attr_x( '\a\t', 'time prefix in comment date', 'junktruck' ) . ' ' . get_option( 'time_format' );
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'junktruck-theme/comments/date', sprintf( 
	...
	return apply_filters( 'junktruck_get_comment_date', sprintf( '<span class="comment-date"><a href="%3$s" class="comment-date__link"><time datetime="%1$s" class="comment-date__time">%2$s</time></a></span>', get_comment_time( 'c' ), get_comment_date( $format ), get_comment_link() ), $args );
}
 ...
 */
function junktruck_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'junktruck-theme/comments/text', $comment_text, $args );
}
 ...
 */
function junktruck_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'junktruck-theme/comments/reply_link', $reply, $args );
}
 ...
 */
function junktruck_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'junktruck' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'junktruck-theme/comments/link-edit', $link, $comment->comment_ID, $args );
}
