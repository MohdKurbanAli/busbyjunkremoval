 ...
 * @return array
 */
function junktruck_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = junktruck_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = junktruck_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = junktruck_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		$block_title = ( $settings['block_title'] ) ? '<h5 class="related-posts__title">' . $settings['block_title'] . '</h5>' : '';
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? junktruck_post_thumbnail( 'junktruck-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? junktruck_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? junktruck_posted_on( array( 'echo' => false ) ) : '';
